...
@Component({
  ...
  template: `
    <div>
      <a>Go Back to the DashBoard</a>
      <!-- hey ! -->
      <a>Go to your wallet</a>
      <!-- look at me -->
      <div *ngIf="(crypto$ | async) as data">
        ...
        <button (onClick)="(nop)">Buy</button>
        <div *ngIf="(nbOfCrypto$ | async) === 0">
          You have no {{ data.name }}
        </div>
        <div *ngIf="(nbOfCrypto$ | async) as nbOfCrypto">
          You have {{ nbOfCrypto }} {{ data.name }}
        </div>
        <p></p>
      </div>
    </div>
